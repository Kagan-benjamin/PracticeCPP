...
void test1(){
    ...
    std::vector<int> vec1(10,100); // ten 100s in vector 
    display(vec1);
}
...
void test4(){
    ...
    stooges.push_back(Person{"Layla",32});
    display(stooges);
    
    stooges.emplace_back("Daniel",26);
    display(stooges);
}
 ...
 * new elements at the end of the container it is applied to. It's 
 * a special output iterator that is very efficient. There is also
 * a front_inserter we can use with deques and lists. Copy one list
 to another using an iterator and back_inserter */
    std::vector<int> vec1{1,2,3,4,5};

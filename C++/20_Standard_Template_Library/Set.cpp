...
void test2(){
    ...
    std::set<Person> stooges{
        {"Larry",1},
        {"Moe",2},         // using operator< to order the set 
        {"Curly",3}
    };
    display(stooges);
    ...
    stooges.emplace("Frank",50);  // not allowed, 50 already exists
    display(stooges);
    ...
    it = stooges.find(Person("XXXX",50));   // will remove James! defined by operator<
    if (it != stooges.end()){
        stooges.erase(it);
    }
    display(stooges);
...
void test3(){
    ...
    auto result = s.insert("D"); // returns std::pair of iterator to D and bool result 
    display(s);
